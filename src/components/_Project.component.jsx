...
import Tracker from "./_Tracker.component"
...
export default class Project extends Component {
		...
		if (!tracker.running) {
			...
			this.ticker = setInterval(() => {
				...
				if (
					...
					store.dispatch({
						type: "ADD_TRACKER",
						payload: { project: project, created: tracker }
					})
	...
	componentDidMount() {
		...
		if (
			this.props.project.trackers.filter(tracker => {
				return isToday(tracker)
			}).length === 0
		) {
			store.dispatch({
				type: "ADD_TRACKER",
				payload: { project: this.props.project.created }
			})
		}
	}
	...
	render() {
		...
		return (
			<div
				className={`project ${trackerRunning() ? "running" : ""}`}
				style={doStyle()}
				id={project.created}
				ref={e => {
					if (e) {
			...
			>
				<div className='top flex row justify-between align-center'>
					...
					<ul className='flex row justify-end align-center'>
						<li>
							<i className={`material-icons schedule `}>
								schedule
							</i>
						</li>
						<li>
				...
				<ul className='bottom flex row justify-between align-center'>
					{!this.ticker ? (
						<li>
							...
							>
								<i className='material-icons abs middle left'>
									play_arrow
								</i>
								<span>Start</span>
							</button>
				...
				>
					<li
						onClick={() => {
							this.handleRemove(project.created)
						}}
					>{`Delete ${project.title}`}</li>
					<li
						onClick={() => {
							this.handleExport(project)
						}}
					>
						Export
					</li>
					...
					<li
						onClick={() => {
							this.addTracker(project.created)
						}}
					>
						Add Tracker
					</li>
